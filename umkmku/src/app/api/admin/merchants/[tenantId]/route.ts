...
export async function GET(_: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const [{ data: tenant }, { data: sub }, { count: productCount }, { count: orderCount }] = await Promise.all([
    ...
    db.from('tenant_subscriptions').select('plan_id, status, trial_ends_at, current_period_start, current_period_end, ai_tokens_used, transactions_used, ai_token_limit_override').eq('tenant_id', tenantId).single(),
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  if (denied) return denied
  const { tenantId } = await params
  const { ai_token_limit_override } = await req.json()
  const db = createServiceClient()
  const { error } = await db
    .from('tenant_subscriptions')
    .update({ ai_token_limit_override: ai_token_limit_override ?? null })
