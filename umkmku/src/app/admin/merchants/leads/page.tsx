...
export default async function LeadsPage() {
  ...
  const { data } = await db
    ...
    .select('id, trial_ends_at, tenants(slug, brand_name, category)')
    .eq('status', 'trial')
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[#5E6B85] mb-6">Merchant yang sedang trial gratis — {data?.length ?? 0} merchant</p>
